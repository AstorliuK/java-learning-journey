    ...
    public void startSystem(){
        ...
        while (num != 6) {
            ...
            case 5:
                ...
                Student select = clazz.selectStudentById(selectId);
                /* 这行代码其实就是命名一个Student类型的select变量。
                 * 然后将clazz.selectStudentById(selectId)，这行内容的结果指向它。
                 * 这并不是创建新对象……就单纯的赋值，不过这行代码是引用，是内存地址的指向。
                 */
                if(select!=null){
                    System.out.println("查询成功！你所查询的学生信息如下：");
                    System.out.println(select.showInfo());
                }
                /* 大晚上的，忽然有点发蒙，要先创建对象才能使用里面的方法啊。
                 * 但这里，虽然select没有new，但是他指向了静态对象clazz啊，即被赋予了内存地址了。
                 * clazz的构造方法中，是有new Student的，所以他也能用showInfo()。
                 */
                break;
            case 6:
